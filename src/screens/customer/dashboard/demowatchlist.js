...
const demowatchlist = (props) => {
    ...
    const containerStyle = { position: 'absolute', borderRadius:10, top: '10%', width: '90%', backgroundColor: '#FFFFFF', padding: '1%', marginHorizontal: '5%', alignItems: 'center' };
    ...
    const renderItem = ({ item, index }) => {
        return (
            <View style={tw.style('flex ml-2 w-40 rounded-lg p-1')}>
                <TouchableOpacity onPress={() => props.navigation.navigate("NameStoredemo", { productId: item._id, userId: item._id, productQuantity: item.productQuantity })}>
        ...
        <KeyboardAvoidingView
                ...
                <Provider>
                    <Portal>
                        <Modal visible={visible} onDismiss={closepopup} contentContainerStyle={containerStyle}>
                            <View style={tw.style('items-center mt-[10%] mb-[15%]')}>
                                  <Logobase />
                            </View>
                            <Text style={styles.Modaltext}>Sign for more features</Text>
            ...
            <View style={tw.style('max-w-fit bg-white flex h-18 px-6 py-4 md:py-4 md:px-6 sm:mx-1 md:mx-2')}>
                    <View style={tw.style('flex-row justify-between')}>
                        ...
                        <TouchableOpacity style={tw.style('px-2')} onPress={() => openpopup() }>
